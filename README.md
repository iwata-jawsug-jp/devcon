...
このプロジェクトを自分で動かすには、公開リポジトリ（`iwata-jawsug-jp/devcon`）を **fork**
して使う。アプリをローカルで動かすだけなら AWS は不要（下記「クイックスタート（ローカル）」）。
CI/CD で自分の AWS にデプロイするには「本格セットアップ（自分の AWS で実開発）」を行う。
...
├── services/
│   ├── api/            # バックエンド REST API（Python / FastAPI / uv 管理）
│   └── web/            # フロントエンド SPA（TypeScript / Vite + Vue 3）
...
- `web`（静的 SPA）と `api`（ステートレス JSON API）は別プロセス。ブラウザは相対パス
  `/api/*` で API を呼ぶ。
...
2. **`infra/bootstrap/` を一度だけ apply** して、OIDC プロバイダ・CI IAM ロール・state バケットを
   作る。このとき `github_org` / `github_repo` を**自分の fork**（`<your-org>/<your-repo>`）に
   設定する。OIDC trust がこのリポジトリに限定されるため、**既定値のままだと fork 先の CD が
   認証で失敗する**。
3. bootstrap の出力を**リポジトリ変数**に登録する：`AWS_TF_STATE_BUCKET` /
   `AWS_PLAN_ROLE_ARN` / `AWS_DEPLOY_ROLE_ARN`。**未登録だと `cd-infra.yml` の plan/apply は
   OIDC 認証段階で失敗する**（想定挙動）。
...
```bash
make help        # 全コマンド一覧
make dev         # api(:8000) + web(:5173) を同時起動
make gen-types   # API の OpenAPI から web の TS 型を生成
...
#### バックエンド API（services/api · FastAPI）
...
#### フロントエンド SPA（services/web · Vite + Vue 3）
...
- `ci.yml` … PR / main push で変更パスのみ per-service ジョブ（api / web / infra）。
- `cd-infra.yml` … PR で `terraform plan`、main マージで `apply`（`production` 環境で承認ゲート）。
- `cd-app.yml` … api イメージを ECR へ push し ECS を更新、web を S3 同期 + CloudFront 無効化。
