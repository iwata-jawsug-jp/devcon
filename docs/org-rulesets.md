...
> 承認のうえ Claude Code が `gh api` で作成）。適用直後に `devcon` への `publish.yml` の直接 push
> がブロックされる問題が発覚し、`devcon` を除外して修正済み（後述）。適用状況は下記
> 「適用コマンド」節の確認コマンドでいつでも再確認できる。
...
対象は `~ALL`（org 内の全リポジトリ）から、`devcon` を明示的に除外する。

> **`devcon` を除外する理由（実装中に発見）:** `devcon`（公開ミラー）は `publish.yml` が
> deploy key で `main`・タグへ**直接 push**する運用（1リリース = 1スナップショットコミット、
> `docs/release.md`）のため、「PR 必須・直push禁止」を適用すると
> `GH013: Repository rule violations`（`Changes must be made through a pull request`）で
> リリース公開そのものが失敗する（v0.3.9 で実際に発生・要修正）。他の PR ベースで開発する
> リポジトリが増えたら、それらは `~ALL` に含まれたまま自動的に対象になる。
...
gh api -X POST orgs/iwata-jawsug-jp/rulesets \
  ...
  -F 'conditions[repository_name][exclude][]=devcon' \
