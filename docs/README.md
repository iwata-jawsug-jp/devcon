...
| [frontend-design.md](frontend-design.md)                     | フロントエンドのデザイントークン（design.md） — ブランドカラー・フォントなど `services/frontend/` の最小限の見た目定義                                                          |
...
| [scaffold-cli.md](scaffold-cli.md)                           | スキャフォールドCLI（copier）— `copier.yml` のテンプレート化・変数設計・生成物検証CIの設計ノート（#294）                                                                        |
...
| [org-rulesets.md](org-rulesets.md)                           | 組織レベルの GitHub Rulesets 標準セット — `iwata-jawsug-jp` org 全体へ適用済み（#295）、設計判断と適用/確認コマンド                                                             |
...
| [metrics/](metrics/)                                         | DORA メトリクス・プラットフォーム成熟度スコアカードの月次スナップショットと評価基準（`metrics-dora.yml`/`metrics-scorecard.yml` が生成）                                        |
