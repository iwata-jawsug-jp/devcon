...
- **2 層構成で配線する**（#285 で明文化した bootstrap の CI 外運用と同じ考え方）:
  ...
  - **bootstrap 層**: `make check-iam-policies`（ローカル専用、要 AWS 認証）。対象は
...
- **受け入れるコスト:**
  ...
  - bootstrap 層の 9 ポリシーは CI では検証されず、ローカル実行（`make check-iam-policies`）を
    忘れると見逃される。忘れないための機械的な強制（pre-commit 等）は、bootstrap が CI 外で
    ローカル AWS 認証を前提とする構造上、現状は導入していない。
- **再検討トリガー:**
