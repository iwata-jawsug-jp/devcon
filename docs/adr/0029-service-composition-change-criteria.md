...
## 追記（2026-08-02）: backend 品質ゲートパリティの非同期ロールへの適用範囲

Decision 項目5「backend Dockerfile の最低限契約」6点は ECS Fargate 常駐サーバー（同期REST）を
前提とした契約であり、非同期/イベント駆動ロール（Lambda、ADR-0024）には同じ形で当てはまらない
（`docs/service-replacement-guide.md`「backend 置き換え手順」冒頭の対象外注記を参照）。一方、
同じ項目5が定める「backend 品質ゲートパリティ」（lint・型チェック・カバレッジの3種）は実行基盤に
依存しない不変条件であり、非同期/イベント駆動ロールにも適用される——この区別が本 ADR 初版の文面
からは読み取りにくかった（`itouhi/java-webapp3#1`、devcon#770 で判明）。

`services/backend/go`（Lambda-only）が golangci-lint・`go test` で既にこの3種のゲートを満たして
いることが実例である。Dockerfile契約6点とは異なり、品質ゲートパリティは「HTTPサーバーとして
稼働するか」ではなく「backend 実装であるか」にのみ依存するため、非同期ロールを対象外にしない。
