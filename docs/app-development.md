...
### ロギング（構造化ログ + リクエスト相関ID、#42）

- `api/logging_config.py` の `configure_logging()`（`main.py` の `create_app()` から起動時に
  1回呼ぶ）が root logger を JSON 1行ログ（`JsonFormatter`）に差し替える。標準の
  `logging.getLogger(__name__).info(...)` 等をそのまま使えば自動的に JSON 化される。
- `api/middleware.py` の `CorrelationIdMiddleware` が各リクエストに相関ID（クライアントの
  `X-Request-ID` があればそれを使用、無ければ生成）を割り当て、
  `logging_config.request_id_ctx`（`contextvars`）経由でリクエスト中のどのログ呼び出しにも
  自動的に載せる（引数で渡し回す必要がない）。レスポンスヘッダーにも `X-Request-ID` を
  エコーバックするので、フロント側のログ・ネットワークタブと突き合わせられる。
- 本番イメージ（`Dockerfile`）は uvicorn 自身のアクセスログ（非JSON、重複）を
  `--no-access-log` で無効化している。ローカル開発（`make dev`）は人間が読みやすいよう
  そのまま uvicorn の既定ログを使う。
- **分散トレーシング**（[ADR-0007](adr/0007-opentelemetry-adot-sidecar-for-distributed-tracing.md)）:
  `api/tracing.py` の `configure_tracing()` が `API_OTEL_TRACES_ENABLED=true`（インフラの
  `var.otel_traces_enabled`、prod既定）のときだけ FastAPI + SQLAlchemy を計装し、ECSタスクの
  ADOTサイドカー（`localhost:4317`）経由でAWS X-Rayにトレースを送る。有効時は
  `JsonFormatter` が `trace_id`/`span_id` もログに載せる（`request_id` とは別軸、
  X-Rayコンソールへ直接ジャンプするためのID）。ローカル開発では既定で無効
  （コレクタが無いため）。
...
`perf/k6/items-smoke.js` が items API に対する代表シナリオ（health → list → get → create）を
一定VU数で回し、しきい値（p95レイテンシ・エラー率）で合否判定する。**PRごとのCIゲートには
含めない**（`.github/workflows/perf.yml`、手動 `workflow_dispatch` のみ）— 重い上に
共有ランナーでのノイズで偽陽性になりやすいため。定期（`schedule`）実行は現状無効化しており、
本番運用のアプリで再有効化する手順は
[infrastructure.md の「定期実行ワークフローについて」](infrastructure.md#定期実行ワークフローについてmetrics-dorayml--perfyml)参照。

- 対象は `services/backend/python/perf/app.py`（本番の`api.main.app`をラップし、
  `get_current_user`を差し替えて認証をスタブ化したもの。**本番コードには含まれない**
  ——`Dockerfile`は`src/`のみをコピーするため）。JWT検証（Cognito依存）の待ち時間ではなく、
  **自前のAPI（ルーティング・バリデーション・リポジトリ/DB層）の性能**を計測する意図的な
  スコープ限定。
- しきい値は`infra/variables.tf`の`alarm_alb_latency_seconds`（既定1秒）を暫定のSLO代替として
  採用。#42のSLO/SLI定義（フェーズ4）が確定次第、突き合わせて調整する。
- ローカル実行: `make db-up && make perf-test`（`k6`がPATHに必要）。
