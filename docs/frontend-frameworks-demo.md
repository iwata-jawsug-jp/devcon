...
[sandbox.md](sandbox.md) の「恒久保持の例外（`sandbox/ec-site-demo`）」と同じ方式を踏襲する。
...
- 本番は Vue 3 のまま変更しない。複数フレームワーク対応は本番の要件ではなく、あくまで
  比較・学習のためのサンドボックス活動。
- `sandbox/*` の隔離ポリシー（[sandbox.md](sandbox.md) 参照）により、このブランチから
...
- [sandbox.md](sandbox.md) — `sandbox/*` 隔離ブランチの方式全般、`sandbox/ec-site-demo` の
  恒久保持例外
