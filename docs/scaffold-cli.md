...
- **AWS アカウント ID:** リポジトリ内にハードコードはなし。`infra/bootstrap/iam-ci-roles.tf` の OIDC
  信頼ポリシーは `local.repo`（`var.github_org`/`var.github_repo` 由来、`infra/bootstrap/locals.tf`
  で定義）、`infra/main.tf` の
...
- GitHub リポジトリ変数: `AWS_DEPLOY_ROLE_ARN`、`ECR_REPOSITORY`、`INFRA_APPLY_ENABLED`、
  `LIVE_SMOKE_ENABLED` 等（全量は `docs/repository-variables.md` 参照）
