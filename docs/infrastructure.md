...
- **CI 用 IAM ロール 2 つ + エージェント専用 IAM ロール 1 つ（#571）**
  - `*-ci-plan` … PR 用の読み取り専用（plan）。AWS 管理の `ReadOnlyAccess`。
    ...
    加えて `access-analyzer:ValidatePolicy`（`ci_plan_access_analyzer`）を明示的に付与
    （[ADR-0009](adr/0009-iam-access-analyzer-static-policy-validation.md)、#340）:
    `ReadOnlyAccess` が暗黙にカバーしている可能性はあるが、それに賭けず明示的に許可する。
    リソース ARN を持たない読み取り専用 API のため `Resource "*"`。
  - `*-ci-deploy` … main / `production` 用のデプロイ。**最小権限化済み**（#45）: 以前は
    ...
    この `aws:RequestedRegion` 条件は18ステートメントに同一の4行がコピペされていたため
    （#285）、`local.region_condition` + `dynamic "condition"` ブロックへリファクタ済み
    — リージョン設定を変更する際は `bootstrap/locals.tf` の該当 `locals` ブロック1箇所を
    直せば全ステートメントに反映される。
    **注意**: `bootstrap/` は `cd-infra.yml` の管理対象外 — このポリシー変更は
    ...
    該当アクションを追加すること。
    **運用方針（#285）**: `infra/*.tf` にリソースを追加・変更した際、各
    `infra/bootstrap/iam-ci-deploy-*.tf` の対応する `ci_deploy_*` ステートメントの
    アクション列挙を機械的に追従させる仕組みは
    現状ない（`bootstrap/` は独立適用のため、`infra/*.tf` の変更を検知する CI フックを
    持たせにくい）。単独運用者体制の間は「デプロイ時に `AccessDenied` が出たら
    このファイルを見に行って該当アクションを追加する」運用（上記の注意書きと同じ）で
...
（`local.name_prefix`、locals.tf、#571）。`<suffix>`は`var.resource_name_suffix`（ランダム6文字、
...
>
> `deploy_subjects`（`bootstrap/locals.tf`）は `ref:refs/heads/main` / `environment:production` に
...
> **新規 AWS アカウント/リージョンでの前提条件**: `bootstrap/iam-ci-deploy-data.tf` は `ci_deploy` の
...
tools/script/bootstrap.sh update              # infra/bootstrap/*.tf変更後の再適用（パラメータ不要）
...
`infra/bootstrap` の Terraform state はローカルのみ（リモートbackendを持てない理由は
`infra/bootstrap/README.md` 参照）で、`init`/`update`/`destroy` は `init` を実行した1台の
...
```bash
tools/script/bootstrap.sh adopt               # 他PCのbootstrap設定をリポジトリ変数から取り込む
```
...
実在確認したあと、`terraform init` + このレイヤーの全 managed resource（state バケット一式・
...
- `state_bucket_name` → **リポジトリ変数** `AWS_TF_STATE_BUCKET`、bootstrap の `project`
  ...
  IAM ポリシー側（`bootstrap/iam-ci-roles.tf` の `tfstate_access_plan`）と同じ `project` 値で
...
分散トレーシング（`var.otel_traces_enabled = true` の環境、prod 既定）が有効な環境で、
1 リクエストを web→api→DB まで追う手順。

1. **AWS コンソール → CloudWatch → X-Ray traces**（リージョンは `ap-northeast-1`）を開く。
   - **Service map**: `api`（`otel_service_name` 既定値、`services/backend/python/src/api/config.py`）
     ノードが表示され、DB（RDS）呼び出しがエッジとして繋がっていることを確認できる。
     エラー率・p50/p95 レイテンシもノード単位で見える。
   - **Traces**: 時間範囲を絞って一覧表示。個々のトレースを開くとタイムライン
     （web→api→DB の各スパン、所要時間）が見える。
2. **特定のリクエストのトレースを追いたい場合**は、構造化ログ（CloudWatch Logs、
   ロググループ `/${var.project}/${var.environment}` = `shared.tf`）を先に見る:
   - CloudWatch Logs Insights で `fields @timestamp, trace_id, request_id, message | filter ...`
     のようなクエリを実行し、該当リクエストの `trace_id`（`api/logging_config.py` が
     スパンから埋め込む16進32桁の値）を取得する。
   - X-Ray コンソールの **Traces** 検索ボックスにその `trace_id` を貼り付けて直接ジャンプする。
   - `request_id`（`CorrelationIdMiddleware`）はクライアント/APIログ側の相関ID、`trace_id` は
     X-Ray 側の相関IDという役割分担（ADR-0007）。エラー調査はまず `request_id` でログを
     絞り込み、そこから `trace_id` で X-Ray のタイムラインへ、という順で辿ると効率が良い。
3. **ダッシュボード**（`terraform output cloudwatch_dashboard_url`）と合わせて見ると、
   「アラームが鳴った時間帯のトレースをX-Rayで開く」という流れで障害調査ができる。

**現状のOBS-02判断（追加ダッシュボードの要否）**: ADR-0007はX-Ray単独構成を意図的に選択済み
（Grafana等の追加ダッシュボードは導入しない）。上記のX-Rayコンソール標準機能（Service map /
Traces）で1リクエストを追う完了条件は満たせており、追加ダッシュボードが必要な具体的なニーズは
現時点でない。**要件が明確になった場合**（例: 複数サービスに拡張して横断ダッシュボードが要る、
X-Ray保持期間・クエリ機能の制約に当たる等）に再検討する — それまでは対象外の判断を維持する。
...
- **smoke-test**（第4のゲート、#373/#376、ADR-0008）: `deploy-api`/`frontend` 成功後、
  ...
  `infra/bootstrap/iam-ci-deploy-auth.tf` の `ci_deploy_auth`）。失敗時は run URL・デプロイ SHA・失敗した
...
`cd-sandbox-cycle.yml`（週次エフェメラルサイクル、#376 PR④）も同様に `workflow_dispatch` のみで
`schedule` を持たない。理由はここに挙げた2つに加えてもう1点（`TF_ENV=sandbox` が単一の共有
state のため、無人の定期teardownが手動検証中の環境を破棄しかねない）ある — 詳細は
[sandbox.md](sandbox.md#週次エフェメラルサイクル376-pr④)参照。

### CI実行時間トレンドの可視化（CI-01、#409）

**判断: 現時点では実装しない。** 理由:

- 実際の `ci.yml` の実行時間を確認したところ、全ジョブ（`changes`/`backend`/`frontend`/`infra`/
  `scripts`）が並列実行で概ね3分程度で完走しており（2026-07-13時点の実行実績）、体感的な
  ボトルネックは無い。
- 上記「定期実行ワークフローについて」と同じ理由で、この monorepo は学習・デモ目的で実トラフィックが
  なく、コミット頻度もバースト的（活発な日と非活動日の差が大きい）なため、`metrics-dora.yml`と
  同様の週次集計では意味のあるトレンドが貯まりにくい。
- 現状困っていない指標を先回りして可視化しても、監視対象が1つ増えるだけで運用コストに見合わない
  （「まず観測、しきい値は後で」と同じ思想だが、観測する動機（実際に遅いという痛み）自体がまだ無い）。

**再検討トリガー**: CI実行時間が体感的に伸びた（新しい重いテストスイート追加等）、または
実行時間トレンドを見たいという具体的な必要性が生じた場合。実装する場合は
`metrics-dora.yml`と同じ枠組み（GitHub Actions APIでジョブ実行時間を集計、
`docs/metrics/`へスナップショット保存）を流用できる。

### pre-commitフックの定着状況（CI-02、#409）

**確認結果（2026-07-13時点）: 定着している。** 現在の開発環境で `.git/hooks/pre-commit` が
インストール済み（`make hooks` 実行済み、フック本体に pre-commit 生成のマーカーコメントを確認）。

現状は単独運用者体制のため「定着」は実質「唯一の開発環境で有効化されているか」に等しく、これは
`tools/script/check-devenv-setup.sh`（`make check-setup`）が既に機械的にチェックする項目
（「依存関係・pre-commit hooks（make setup）」セクション）と重複する。新たな確認の仕組みを
追加する必要はなく、既存の `make check-setup` をオンボーディング/環境再構築のたびに実行する
運用で十分と判断する。複数人運用に移行する場合は、CI側での強制（例: pre-commitと同じチェックを
`ci.yml` の `scripts` ジョブ等でも実行し、フック未実行でもCIで検出できるようにする）を再検討する。
