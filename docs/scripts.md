...
| `check-tree-health.sh`     | 任意のツリーに対する内部整合チェック（Markdown相対リンク切れ、`--diff` で2ツリーのファイル一覧差分）。ツリーの種類に依存しない汎用チェッカー（#699）。                                                                                                                                                                                                               | `check-tree-health.sh <dir> [--label LABEL] [--strict]`                                                   |
| `verify-tree-health.sh`    | `check-tree-health.sh` を使い、開発用ツリー・公開用ツリー・その2つを起点に copier 生成した生成先ツリー計4本を一通り検査するオーケストレーションスクリプト（#699）。`make tree-health`/CIは既定で `--strict`（Epic #698 完了によりblocking、#704）。                                                                                                                  | `make tree-health`（CI: `ci.yml` の `tree-health` ジョブ、blocking）                                      |

詳細: [scaffold-cli.md](scaffold-cli.md)、[ci-cd-area-switches.md](ci-cd-area-switches.md)（`TREE_HEALTH_ENABLED`）、[ADR-0028](adr/0028-template-and-generated-asset-separation.md)。
