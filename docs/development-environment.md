...
check-setup` は該当項目を「未登録」ではなく「権限不足で確認できない」旨のスキップ表示に
する（誤って未登録扱いにはしない）。正確に判定したい場合のみ、`.env.check-setup.example`
の手順に従って確認専用の最小権限トークン（`Administration: Read-only` + `Variables:
Read-only`、対象リポジトリ限定・短期の有効期限）を `.env.check-setup` に用意すると、
その項目だけそのトークンで検証できる（用意しなくても `make check-setup` 自体は動く）。
