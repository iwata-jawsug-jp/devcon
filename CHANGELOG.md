...
- **backend の第二言語として Go を採用する提案書と ADR-0024 を追加**: `services/backend/go/` への導入に向け、開発環境（huma v2 + chi によるコードファースト OpenAPI 生成で `make gen-types` パイプラインを言語非依存に維持、pgx + sqlc）・CI/CD（golangci-lint、govulncheck、既存の Makefile 3 層同期・エリアスイッチへの統合）・AWS Lambda 利用時の構成をインターネット調査に基づき検討した。全論点を決定済み: DB スキーマ管理は Python（Alembic）に一元化（Go はマイグレーションを持たず `make gen-schema` のスナップショットを sqlc に供給）、実行基盤は Go を AWS Lambda 専用とし ECS Fargate は Python 専用のまま維持、呼び出し方式は外部 = CloudFront→Function URL(OAC) / 非同期 = SQS・EventBridge・S3 / 内部サービス間 = VPC Lattice のハイブリッド、Python とのすみわけは役割分担型（同期 REST = Python、非同期・イベント駆動 = Go）、`make gen-types` は両サービスの OpenAPI を統合、スキャフォールド CLI テンプレートにも Go を含める（#634, #635）。
- **`cd-sandbox-cycle.yml` に共有 Terraform state の誤破棄防止ガードを追加**: `TF_ENV=sandbox` は手動検証用の `cd-infra-sandbox.yml` と単一の state を共有しており、週次エフェメラルサイクルの `apply` が人間の検証環境や完走しなかった前回実行を巻き込んで上書き・破棄してしまう事故を防ぐ機械的なガードが無かった。`apply` 前に `terraform state list` で state が空かを確認し、非空なら `confirm_shared_state=proceed` を明示指定しない限り停止する（`cd-infra-sandbox.yml` の `confirm_destroy` と同じタイプ入力による誤操作防止パターン）。`teardown` ジョブも `apply` が実際に `terraform apply` まで到達した場合のみ実行されるようゲートし、ガードが止めた場合に `teardown` だけが独立に走って共有 state を破棄してしまう欠陥を防いだ。sandbox 実機検証（実 AWS、3 回の通し実行）で、当初の判定方式（`terraform plan` の差分ベース）が `hashicorp/setup-terraform` のラッパーの既定動作により機能しないこと、さらに `data.external.api_current_image`（#374）の設計と衝突し実際に使われている環境をほとんど検出できない致命的な欠陥を含むことが判明し、`terraform state list` による state 非空判定に設計変更した（#631, [ADR-0025](docs/adr/0025-cd-sandbox-cycle-shared-state-guard.md)）。

## [0.6.8] - 2026-07-23

### Added

- **`write-cd-app-vars.sh`に`--clear`モードを追加し、sandbox環境teardown後にリポジトリ変数を削除できるようにした**: `cd-infra-sandbox.yml`の`destroy`ジョブでsandbox環境をteardownしても、`SANDBOX_*`リポジトリ変数(ECR/ECS/S3/CloudFront/Cognito識別子)は削除されずに残っており、実在しないAWSリソースを指したまま`cd-app-sandbox.yml`の`preflight`が`configured=true`と誤判定し、`build`/`frontend`段階の`NoSuchBucket`/`repository does not exist`という分かりにくいエラーで初めて気づく事故が実際に発生した。`gh variable delete`で対象環境の12変数を削除する`--clear`モードを追加し(terraform output読み取り・AWS認証は不要)、`destroy`ジョブのjob summaryに実行を促すリマインダーも追加した。`GITHUB_TOKEN`はリポジトリ変数への書き込み権限を持たないため、CI内での自動実行ではなく既存の`GH_CHECK_SETUP_TOKEN`(ADR-0021/ADR-0022)を使うローカル実行を採用し、新規シークレットの追加を避けた（#631）。

### Fixed

- **`cd-app.yml`/`cd-app-sandbox.yml`の`preflight`ジョブが`check-app-vars`(composite action)呼び出し前の`actions/checkout`欠落で失敗する不具合を修正**: #617 Phase 2(v0.6.7)で`preflight`をcomposite action呼び出しに置き換えた際、ローカルaction(`uses: ./path`)が実行前にリポジトリのチェックアウトを必要とすることを見落としていた。main上で`cd-app.yml`が3回連続失敗し、公開ミラー(`iwata-jawsug-jp/devcon`)にも伝播した。sandbox実機検証で`preflight`のgreen復帰を確認した（#629）。
