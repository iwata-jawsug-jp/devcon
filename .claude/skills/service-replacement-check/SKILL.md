...
| 5 | Exposes a migration entrypoint invocable via container-command override | Confirm there's a command equivalent to `["uv","run","--no-sync","alembic","upgrade","head"]` and that `.github/workflows/reusable-app-deploy.yml`'s `containerOverrides` (currently line ~164) has been updated to it; for Spring Boot with Spring Security, confirm the entrypoint boots with `server.port=-1`, not `WebApplicationType.NONE` (`service-replacement-guide.md` §3) |
...
| 6 | No `.pre-commit-config.yaml` local hook silently stopped firing | Read every local hook's `files:` regex; for any that targeted a path/format the replacement moved, renamed, or reformatted (e.g. `check-oauth-scopes` vs. `services/frontend/src/auth/oidcConfig.ts`, #756), confirm the pattern still matches — a non-matching pattern fails silently (no error, no warning), not loudly |
