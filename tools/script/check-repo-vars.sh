...
# 全35変数を対象にした横断チェック。CI では使わない — ローカル/devcontainer 専用。
#
...
SWITCH_VARS=(BACKEND_ENABLED BACKEND_GO_ENABLED FRONTEND_ENABLED INFRA_ENABLED SCAFFOLD_ENABLED INFRA_APPLY_ENABLED LIVE_SMOKE_ENABLED)
...
if [[ "$registered_available" == true ]]; then
  ...
  section "登録状況（エリア別/オプトインスイッチ・任意7個）"
